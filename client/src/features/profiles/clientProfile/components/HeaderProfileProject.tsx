...
import { FaCamera, FaEdit, FaPlus, FaUser } from 'react-icons/fa';
import type { HeaderProfilePropsProject } from '../../type';
import { useImageUpload } from '../../../../hooks/useImageUpload'; 

const HeaderProfileProject = ({
  profile,
  setProfile,
  addProjectMode,
  setAddProjectMode,
  ...
  const handleAddProjectClick = () => {
    ...
    setAddProjectMode(!addProjectMode);
  };
      ...
      <div className="relative px-8 pb-8">
        <div className="flex justify-between items-start -mt-16 mb-6">
          ...
          {isOwnProfile && (
            <div className="flex gap-3 absolute right-2 top-4">
              <Button
                text={editProfileMode ? 'Cancel' : 'Edit Profile'}
                variant={editProfileMode ? 'blue' : 'blue'}
                icon={editProfileMode ? <FaEdit /> : <FaUser />}
                onClick={handleEditProfileClick}
                className='w-20 h-14 p-1 '
              />
              <Button
                text={addProjectMode ? 'Cancel' : 'Add Project'}
                variant={addProjectMode ? 'blue' : 'green'}
                icon={addProjectMode ? <FaEdit /> : <FaPlus />}
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-gray-600 text-lg font-medium">{profile.email}</p>
...
export default HeaderProfileProject;