...
export const likePost = async (req, res) => {
  try {
    ...
    const existing = await PostLikeModel.runRawQuery(
    ...
    const softDeleted = await PostLikeModel.runRawQuery(
...
export const unlikePost = async (req, res) => {
  try {
    ...
    const like = await PostLikeModel.runRawQuery(
      `SELECT * FROM post_likes WHERE post_id = $1 AND user_id = $2 AND deleted_at IS NULL`,
      [postId, userId]
    );
...
export const hasUserLikedPost = async (req, res) => {
  try {
    ...
    const liked = await PostLikeModel.runRawQuery(
...
export const getLikesForPost = async (req, res) => {
  try {
    ...
    const likes = await PostLikeModel.runRawQuery(
      `
